...
class INTP_Model(nn.Module):
    ...
    def __init__(self, settings, device):
        ...
        self.transformer_a = Encoder(
            d_model=settings['trans_embedding_dim'], nhead=settings['num_trans_head'], num_encoder_layers=settings['num_trans_layers'], 
            dim_feedforward=settings['trans_feedforward_dim'], dropout=settings['trans_dropout'], k_mode=0
        ).to(self.device)

        self.embedding_layer_b = torch.nn.Linear(settings['trans_embedding_dim'], settings['trans_embedding_dim']).to(self.device)
        self.transformer_b = Encoder(
            d_model=settings['trans_embedding_dim'], nhead=settings['num_trans_head'], num_encoder_layers=settings['num_trans_layers'], 
            dim_feedforward=settings['trans_feedforward_dim'], dropout=settings['trans_dropout'], k_mode=0
        ).to(self.device)
    ...
    def forward(self, batch):
        ...
        a = self.transformer_a(tokens_emb_a, None, attention_mask)
        b = self.transformer_b(tokens_emb_b, None, attention_mask)
        a_c = self.transformer_a(tokens_emb_a_c, None, attention_mask)
        b_c = self.transformer_b(tokens_emb_b_c, None, attention_mask)
        ...
        # if self.settings['k'] <= adj_mat.size(-1):
        #     top_values, top_indices = torch.topk(adj_mat, k=self.settings['k'], dim=-1)
        #     row_indices = torch.arange(top_indices.size(1)).unsqueeze(0).unsqueeze(-1).unsqueeze(-1).to(self.device)
        #     row_indices = row_indices.expand(top_indices.size(0), -1, self.settings['k'], -1)
        #     batch_indices = torch.arange(top_indices.size(0)).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).to(self.device)
        #     batch_indices = batch_indices.expand(-1, top_indices.size(1), self.settings['k'], -1)
        #     top_indices_with_row = torch.cat((batch_indices, top_indices.unsqueeze(-1), row_indices), dim=3)
        #     top_values_inplace, top_indices_inplace = torch.topk(adj_mat_inplace, k=self.settings['k'], dim=-1)
        # else:
        #     top_values, top_indices = torch.topk(adj_mat, k=adj_mat.size(-1), dim=-1)
        #     row_indices = torch.arange(top_indices.size(1)).unsqueeze(0).unsqueeze(-1).unsqueeze(-1).to(self.device)
        #     row_indices = row_indices.expand(top_indices.size(0), -1, adj_mat.size(-1), -1)
        #     batch_indices = torch.arange(top_indices.size(0)).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).to(self.device)
        #     batch_indices = batch_indices.expand(-1, top_indices.size(1), adj_mat.size(-1), -1)
        #     top_indices_with_row = torch.cat((batch_indices, top_indices.unsqueeze(-1), row_indices), dim=3)
        #     top_values_inplace, top_indices_inplace = torch.topk(adj_mat_inplace, k=adj_mat_inplace.size(-1), dim=-1)
