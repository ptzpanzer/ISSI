...
def get_hpcheader(full_setting):
    coffer_slot = full_setting['coffer_slot']
    model = full_setting['model']
    dataset = full_setting['dataset']
    holdout = full_setting['holdout']
...
mkdir -p $TMPDIR/{dataset}/pkl/
tar -C $TMPDIR/{dataset}/pkl/ -xzf $(ws_find blah)/{dataset}/pkl/4_{holdout}_train_-1.tgz
tar -C $TMPDIR/{dataset}/pkl/ -xzf $(ws_find blah)/{dataset}/pkl/4_{holdout}_test_0.tgz
tar -C $TMPDIR/{dataset}/pkl/ -xzf $(ws_find blah)/{dataset}/pkl/4_{holdout}_eval_0.tgz
tar -C $TMPDIR/{dataset}/pkl/ -xzf $(ws_find blah)/{dataset}/pkl/4_{holdout}_eval_20.tgz
tar -C $TMPDIR/{dataset}/pkl/ -xzf $(ws_find blah)/{dataset}/pkl/4_{holdout}_eval_50.tgz
"""
        return hpc_header
...
def wrap_task(config=None):
    with wandb.init(config=config):
        ...
        if not model_available or not dataset_available or not fh_available:
            ...
            job_script = f"{get_hpcheader(full_setting)}\npython {myconfig.train_script_name} $job_id $TMPDIR '{json.dumps(full_setting)}'"
